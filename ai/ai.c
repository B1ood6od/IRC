				...
				if (!strncmp(buf, "PING", 4)) {
					...
					if (!strncmp(command, "001", 3) && channel != NULL) {
						...
						if (!strncmp(message, "!ping", 5)) {
							...
							raw("PRIVMSG %s :-------------------------------------------------------------\r\n", target);
						} if (!strncmp(message, "!blood quit", 11)) {
							raw("PRIVMSG %s :Goodbye %s, I will be back soon.\r\n", target, target);
							return 0;
						}
						...
						// What is
						else if (!strncmp(message, "What is", 7) || !strncmp(message, "what is", 7)) {
							query = message + 8;
							raw("PRIVMSG %s :Hey there %s, you asked %s.\r\n", target, user, query);
						}
