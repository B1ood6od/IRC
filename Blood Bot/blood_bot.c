...
//						raw("PRIVMSG %s :-----------------------------------------------------------\r\n", channel);
						...
						} else if (!strncmp(message, "!math", 5)) {
							raw("PRIVMSG %s :Testing math magic on %s.\r\n", target, message);
							// Parse command to get math, do math, return result
						} else if (!strncmp(message, "!blood quit", 11)) {
							raw("PRIVMSG %s :EMERGENCY SHUTDOWN\t\n", target);
							return 0;
						} else if (!strncmp(message, "!blood curl", 11)) {
							raw("PRIVMSG %s :Reciving page...\r\n", target);
							comm = system("curl https://google.com");
							raw("PRIVMSG %s :Result:\n%s\r\n", comm);
						}
